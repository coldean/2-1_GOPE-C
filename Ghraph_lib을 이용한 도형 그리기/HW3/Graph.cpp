...
void Box::draw_lines() const
{
	if (fill_color().visibility()) {	// fill
		...
		fl_pie((point(0).x), (point(0).y), r + r, r + r, 0, 360); //topleft
		fl_pie((point(0).x) + w - (r + r), (point(0).y), r + r, r + r, 0, 360); //topright
		fl_pie((point(0).x), (point(0).y) + h - (r + r), r + r, r + r, 0, 360); //bottomleft
		fl_pie((point(0).x) + w - (r + r), (point(0).y) + h - (r + r), r + r, r + r, 0, 360); // bottomright
	...
	if (color().visibility()) {	// edge on top of fill
		...
		fl_line((point(0).x) + r, (point(0).y) + h - 1, (point(0).x) + w - r, (point(0).y) + h - 1); //bottom
		fl_line(point(0).x, (point(0).y) + r, point(0).x, (point(0).y) + h - r); //left
		fl_line((point(0).x) + w - 1, (point(0).y) + r, (point(0).x) + w - 1, (point(0).y) + h - r); //right

		fl_arc(point(0).x, point(0).y, r + r, r + r, 90, 180); //top left
		fl_arc((point(0).x) + w - (r + r), point(0).y, r + r, r + r, 0, 90); //top right
		fl_arc(point(0).x, (point(0).y) + h - (r + r), r + r, r + r, 180, 270); //bottom left
		fl_arc((point(0).x) + w - (r + r), (point(0).y) + h - (r + r), r + r, r + r, 270, 360); //bottom right
	}
}
...
void Regular_polygon::draw_lines() const
{
	...
	if (fill_color().visibility()) {	// fill
		...
		for (int i = 0; i < number_of_points() - 1; ++i) {
			fl_vertex(point(i + 1).x, point(i + 1).y);
		}
		fl_end_polygon();
	...
	if (color().visibility()) {	// edge on top of fill
		...
		fl_line(point(1).x, point(1).y, point(n).x, point(n).y);
	}
}
...
void Star::draw_lines() const
{
	...
	if (color().visibility()) {	// edge on top of fill
		...
		fl_line(point(1).x, point(1).y, point(2 * n).x, point(2 * n).y);
	}
}
...
void Rose::draw_lines() const
{
	if (fill_color().visibility()) {	// fill
		...
		if (d * n % 2 == 0)
		{
			for (int i = 0; i < 361 * d; ++i) {
				tempx = point(0).x + r * cos((double)((double)n / (double)d) * 3.14159 / 180 * i) * cos(3.14159 / 180 * i);
				tempy = point(0).y + r * cos((double)((double)n / (double)d) * 3.14159 / 180 * i) * sin(3.14159 / 180 * i);
				fl_vertex(tempx, tempy);
			}
		}
		else
		{
			for (int i = 0; i < 181 * d; ++i) {
				tempx = point(0).x + r * cos((double)((double)n / (double)d) * 3.14159 / 180 * i) * cos(3.14159 / 180 * i);
				tempy = point(0).y + r * cos((double)((double)n / (double)d) * 3.14159 / 180 * i) * sin(3.14159 / 180 * i);
				fl_vertex(tempx, tempy);
			}
...
void Cycloid::draw_lines() const
{
	if (color().visibility()) {	// edge on top of fill
		...
		for (int i = 0; i < d; ++i) {
			tempx = point(0).x + r * ((3.14159 / 180 * i) - sin(3.14159 / 180 * i));
			tempy = point(0).y - r * (1 - cos(3.14159 / 180 * i));
			fl_vertex(tempx, tempy);
		}

...
struct Box : Shape {
	...
	{
		if (h <= 0 || w <= 0 || r <= 0) error("Bad box: non-positive values");
		add(xy);
	}
	void draw_lines() const;
...
struct Regular_polygon : Shape {
	...
	Regular_polygon(Point xy, int nn, double rr) : n{ nn }, r{ rr }
	{
		if (n <= 0 || r <= 0) error("Bad polygon: non-positive values");
		add(xy);
		savepoints(nn);
	}
	void draw_lines() const;
...
private:
	int n;
	double r;
	void savepoints(int n) {
		int tempx = point(0).x;
		int tempy = point(0).y;
		for (int i = 0; i < n; ++i) {
			Point p = Point((int)(tempx + r * cos(2 * 3.14159 / n * i)), (int)(tempy + r * sin(2 * 3.14159 / n * i)));
			Shape::add(p);
		}
	}
};
...
struct Star : Shape {
	...
	Star(Point xy, int nn, double rr1, double rr2) :n{ nn }, r1{ rr1 }, r2{ rr2 }
	{
		if (n <= 0 || r1 <= 0 || r2 <= 0) error("Bad star: non-positive values");
		add(xy);
		savepoints(nn);
	}
	void draw_lines() const;
...
private:
	...
	void savepoints(int n) {
		...
		int i = 0;
		while(i < 2 * n) {
			Point p = Point((int)(tempx + r1 * cos(3.14159 / n * i)), (int)(tempy + r1 * sin(3.14159 / n * i)));
			Shape::add(p);
			++i;
			p = Point((int)(tempx + r2 * cos(3.14159 / n * i)), (int)(tempy + r2 * sin(3.14159 / n * i)));
			Shape::add(p);
			++i;
		}
	}
...
struct Rose : Shape {
	...
	Rose(Point xy, int nn, int dd, double rr) : n{ nn }, d{ dd }, r{ rr }
	{
		if (n <= 0 || d <= 0 || r <= 0) error("Bad Rose: non-positive values");
		add(xy);
	}
	void draw_lines() const;
...
struct Cycloid : Shape {

	Cycloid(Point xy, double rr, int dd) : r{ rr }, d{ dd }
	{
		if (r <= 0 || d <= 0) error("Bad cycloid: non-positive values");
		add(xy);
	}
	void draw_lines() const;
